...
//recode
//remake (PoxCraft)
//Nambahin fimtyur(Kahfz Bot)
// sc ini belum gua tambahin fitur karna bukan yg di sc ini 
//jika ingin sc seperti yg ada tambahan fityur beli ae 30k
//jangan jual sc ini🗿👍
...
waktuafk = 'Nothing'
alasanafk = 'Nothing'
NamaBot = settings.NamaBot //jangan di ubah 
NomorOwner = settings.NomorOwner //jangan di ubah
NamaOwner = settings.NamaOwner //jangan di ubah
multi = true
nopref = false
...
try {
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© KUHXBOT_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'bit.ly/ownerpbotpoxcrafts', "groupName": `${NamaBot}`, "footerText": "*_© KUHXBOT_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			"groupJid": "6285387059588-1616169743@g.us",
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© KUHXBOT_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: thubb, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:thubb,sourceUrl:`https://youtube.com/c/KukuhRamadhann`}}})
		}
		const simir = (teks) => {
...
return reply(parse)
           ...
           /*if (budo.startsWith(`>`)){
			...
			if (budy.startsWith('>')){
if (!isOwner)return reply(`Perintah ini tidak bisa dilakukan oleh kamu..`)
...
denz.updatePresence(from, Presence.recording)
   ...
   const sotoy = [
	    ...
	    if (isCmd && !isGroup) {console.log(color('|PoX|', 'greenyellow'), color(moment(mek.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(`${command} [${args.length}]`, 'cyan'), color(`${pushname}`, 'orange'), color(`${sender}`, 'deeppink'))}
...
Good ${tampilUcapan}
Saya kuhxbot Yg Siap 
Membantu Anda
Dalam Membuat Sticker , Dll
Silahkan Gunakan Bot Dengan Baik.

Fitur button menu belum support di whatsapp mod
`
sendButLocation(from, `${menu}`, "*© KUHXBOT*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`${prefix}script`,buttonText:{displayText:'SCRIPT BOT'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
  break
case 'command':
 listMsg = {
 buttonText: 'LIST MENU',
 footerText: '*© KUHXBOT*',
 description: `Hai Kak, Silahkan Pilih MENU BOT Disini..`,
 sections: [
                     {
...
break
//tqto nya jan dihapus la:) udh ada nama lu juga ///msih dihapus:( gw tau si bakal dihapus:')
case 'tqto':
menu = `❏ 「 \`\`\`THANKS TO\`\`\` 」
├────────────────────
├ Devolopers : Zero YT7
├ Penyedia CASE & MODULES
├ Allah S.W.T
├ Owner : ${NamaOwner}
├ All my Friends
├ All Creators Botz
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "https://youtu.be/ZeroYT7", matchedText: 'https://youtu.be/ZeroYT7', description: "", title: "Click here🔥!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/ramadhankukuh/wabotz"
        },
        "index": 0
      }
    ]
...
setTimeout( () => {
	    ...
	    reply(`*Sepertinya dia punya bokev :v*`)
break
case 'buglokasi':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
denz.updatePresence(from, Presence.composing)
